...
import { Class, parseClassesDay } from 'utils'

const parseClassesLine = (line: string, uvsColors: Record<string, classColor>): Class[] | undefined => {
    ...
    if (match?.length) {
        ...
        const week = match[7]
        const day = parseClassesDay(match[8]) ?? ''
        const start = match[9]
        const end = match[10]
        ...
        if(match[13]) {
            const day_2 = parseClassesDay(match[14]) ?? ''
...
export default parseClassesLine
