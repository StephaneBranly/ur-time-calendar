...
const parseLine = (line: string): Class | undefined => {
    const match = line.match(/([A-Z0-9]{4})\s*(([A-Z]{1})\s([0-9]*)|([A-Z]{1})([0-9]*))\s([AB])?\s*([A-Z.]*)\s([0-9]{2}:[0-9]{2})-([0-9]{2}:[0-9]{2}),(F[0-9]),S=([A-Z0-9]*)/)
    if (match?.length)
    {
        ...
        return new Class(UVname, classType, Number(classReference), day, start, end, place, frequence, week as 'A' | 'B' | undefined)
    }
    return undefined
}
